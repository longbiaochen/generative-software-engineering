...
**生成式软件工程（Generative Software Engineering, GSE）** 将软件工作的基本单位定义为：把一个意图端到端地推进为可用、可维护的软件实体，并由同一个核心责任主体完成整个闭环。

```text
意图（Intent） → 可执行 / 可维护的软件实体（Executable / Maintainable Software Entity）
```

一个根 GSE Agent 负责完整闭环：理解意图、检查真实环境、定义结果契约、实现、验证、集成、交付和维护。当并行、专业能力或独立评估确有价值时，才动态创建临时子 Agent。
...
本仓库是 GSE 理论与实践的开放研究平台。我们团队致力于成为 **GSE 方法论的主要提倡者与布道者**，通过这个仓库展示 GSE 在真实工程中的应用、演进和验证。

> 本中文 README 用于快速理解 GSE。规范性定义仍由 [`docs/SPECIFICATION.md`](docs/SPECIFICATION.md) 及其所链接的单一职责文档维护。

## 抽象层正在上移：从"写代码"到"结果工程"

当 Coding Agent 让实现代码越来越便宜，真正稀缺的工程工作会向上游移动。最重要的问题逐渐变成：**目标到底是什么？哪些约束定义了解空间？什么质量与成本的约束真正定义了成功？**

GSE 把"生成代码"看成更大工程闭环中的一种 tactic（实现手段）。代码仍然重要，但 `Prompt → Code` 已经不足以承担真实软件工作的责任边界。更持久的工程价值来自于：**明确结果契约、独立验证、系统集成与可维护性的完整闭环**。

| 抽象层 | 主要工作单位 | 常见停止点 | GSE 的判断 |
| --- | --- | --- | --- |
| 人工编码 | Code | "我已经实现了。" | 实现只是结果的一部分。 |
| AI 代码生成 | Prompt → Code | "模型已经生成补丁。" | 生成不能证明交付。 |
| 生成式软件工程 | Intent → Software Entity | 有证据支撑的可用结果 | 目标、约束、执行、验证、集成与责任属于同一个闭环。 |

这也意味着工程师的角色正在变化。最高杠杆的工作会越来越集中在 **目标定义、系统设计、任务分解、工具与环境设计、验证以及最终责任** 上。GSE 框架帮助团队围绕这些核心工作重新组织。
...
当我们把"生成代码"误认为"交付软件"时，Agentic Software Development 很容易失败：补丁可以编译，但用户路径仍然不可用；测试可以通过，但真实环境中的约束尚未验证；子 Agent 完成了任务，但集成与维护责任仍然悬而未决。
...
这个仓库是一个持续演进的研究与方法项目。我们通过真实案例、对标研究与持续验证，推动 GSE 方法论的完善。规范层面会刻意保持精简：研究笔记可以提出变化，但只有正式提升进入 Specification 的内容才会改变当前共识。

首个公开研究版本为 **v0.1**。研究计划本身应当可证伪：Benchmark 需要保留足够的信息，使不同方法能够公平比较，包括任务输入、基线、运行时 / 模型版本、完整交付物与证据链。
...
GSE 使用双许可证：软件、可执行配置和 Agent Skill 产物采用 **Apache-2.0**；方法论文档、研究材料、模板、图表和案例采用 **CC BY 4.0**。完整条款见 [`LICENSE`](LICENSE)。
