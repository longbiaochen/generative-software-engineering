...
当 Coding Agent 让实现代码越来越便宜，真正稀缺的工程工作会向上游移动。最重要的问题逐渐变成：**目标到底是什么？哪些约束定义了解空间？什么证据足以证明成功？当生成出来的代码不完整、不一致或错误时，谁对最后的软件结果负责？**

GSE 把“生成代码”看成更大工程闭环中的一种 tactic（实现手段）。代码仍然重要，但 `Prompt → Code` 已经不足以承担真实软件工作的责任边界。更持久的责任单位应当是：从意图出发，得到一个真的可运行、可维护，并且有足够证据支撑“已经完成”这一判断的软件实体。
...
这也意味着工程师的角色正在变化。最高杠杆的工作会越来越集中在 **目标定义、系统设计、任务分解、工具与环境设计、验证以及最终责任** 上。GSE 希望把这种变化显式化，并把它变成一套可以讨论、比较和验证的工程方法，而不只是“管理 Coding Agent”的个人经验。
